...
m=$(tail -n +2 $SAMPFILE | while read Strain
do
 ...
 name=$Strain
 proteins=funannotate/${name}/predict_results/$outname.proteins.fa
 if [ ! -f $INDIR/${name}.scaffolds.fasta ]; then
	...
	if [ ! -s funannotate/${name}/predict_results/augustus.gff3 ]; then
		echo "echo funannotate/${name}" >>  delete_$$.sh
		echo "/usr/bin/rm -rf funannotate/${name}/predict_misc/busco*" >> delete_$$.sh
		echo "mv funannotate/${name}/predict_misc/EVM_busco funannotate/${name}/predict_misc/EVM_busco.b" >> delete_$$.sh
		echo "/usr/bin/rm -rf funannotate/${name}/predict_misc/hints.*" >> delete_$$.sh
	fi
	if [ ! -s funannotate/${name}/predict_misc/genemark/genemark.gtf ]; then
		 echo "rm -rf funannotate/${name}/predict_misc/genemark*" >> delete_$$.sh
	fi
	echo $N
fi
 N=$(expr $N + 1)
...
echo 'for file in funannotate/*/predict_misc/EVM_busco.b; do rsync -a --delete ./empty/ $file/; rmdir $file; done' >> delete_$$.sh
